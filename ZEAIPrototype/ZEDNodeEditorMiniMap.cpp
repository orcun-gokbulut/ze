...
void ZEDNodeEditorMinimap::mousePressEvent(QMouseEvent* Event)
{
	if((Event->buttons() & Qt::LeftButton) == Qt::LeftButton)
	{
		...
		SeenRectangleOnView = ParentView->mapToScene(ParentView->rect());

		QRect SeenRectangleOnViewRectangle = QRect(MouseInitialPosition.x() - SeenRectangleOnView.boundingRect().width() / 2, MouseInitialPosition.y() - SeenRectangleOnView.boundingRect().height() / 2, SeenRectangleOnView.boundingRect().width(), SeenRectangleOnView.boundingRect().height());

		ParentView->ensureVisible(SeenRectangleOnViewRectangle.topLeft().x(), SeenRectangleOnViewRectangle.topLeft().y(), SeenRectangleOnViewRectangle.width(), SeenRectangleOnViewRectangle.height(), 0,0);
		SeenRectangleOnView = ParentView->mapToScene(ParentView->rect());
...
void ZEDNodeEditorMinimap::mouseMoveEvent(QMouseEvent* Event)
{
	if((Event->buttons() & Qt::LeftButton) == Qt::LeftButton)
	{
		...
		SeenRectangleOnView.translate((MouseLastPosition.x() - MouseInitialPosition.x()), (MouseLastPosition.y() - MouseInitialPosition.y()));	

		QRectF SeenRectangleOnViewRectangle = (SeenRectangleOnView.boundingRect());
		ParentView->ensureVisible(SeenRectangleOnViewRectangle.topLeft().x(), SeenRectangleOnViewRectangle.topLeft().y(), SeenRectangleOnViewRectangle.width(), SeenRectangleOnViewRectangle.height(), 0,0);
...
void ZEDNodeEditorMinimap::drawForeground (QPainter* Painter, const QRectF &Rect)
{
	...
	Painter->drawPolygon(SeenRectangleOnView);
...
ZEDNodeEditorMinimap::ZEDNodeEditorMinimap(QGraphicsView* View, QGraphicsScene* Scene, QWidget* Parent) : QGraphicsView(Parent)
{
	...
	SeenRectangleOnView = QPolygonF(0);
