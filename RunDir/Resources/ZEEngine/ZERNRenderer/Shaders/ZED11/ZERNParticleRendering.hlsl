...
#ifndef __ZERN_PARTICLE_RENDERING_H__
#define __ZERN_PARTICLE_RENDERING_H__
...
{
	float2 Vertex = ((uint2(VertexID, VertexID + 1) & uint2(2, 2)) + float2(-1.0f, -1.0f));
	...
	Output.Position = Center + ZERNTransformations_ViewToWorld(float4(VertexScaled, 0.0f, 0.0f)); //mul((float3x3)ZERNView_InvViewTransform, float3(Corner, 0.0f));
...
ZERNParticleRendering_PixelShader_Input ZERNParticleRendering_DomainShader_Main(ZERNParticleRendering_HullShader_ConstantData_Output Input, float2 UV : SV_DomainLocation, const OutputPatch<ZERNParticleRendering_DomainShader_Input, 4> Patch)
{
	...
	float3 TexCoord1 = lerp(Patch[0].TexCoord, Patch[1].TexCoord, UV.x);
	float3 TexCoord2 = lerp(Patch[2].TexCoord, Patch[3].TexCoord, UV.x); 
	float3 TexCoord  = lerp(TexCoord1, TexCoord2, UV.y);
	
	float4 PositionWorld = mul(ZERNParticleRendering_WorldMatrix, float4(VertexPosition, 1.0f));
