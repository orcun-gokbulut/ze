...
{
	return max(dot(Color, ZERNHDR_LuminanceWeights), 0.00001f);
}
...
{
	float A = 0.10f;	//Shoulder Strength
...
float3 ZERNHDR_Bright_Calculate_PixelShader(float4 PositionViewport : SV_Position, float2 TexCoord : TEXCOORD0) : SV_Target0
{
	...
	for (uint I = 0; I < 4; I++)
	{
		...
		for (uint S = 0; S < SAMPLE_COUNT; S++)
		{
			...
			ScaledLuminance = log2(max(ScaledLuminance, 0.00001f));
...
float4 ZERNHDR_ToneMapping_PixelShader(float4 PositionViewport : SV_Position, float2 TexCoord : TEXCOORD0) : SV_Target0
{
	float3 ResultColor = 0.0f;
#ifdef MSAA
	[unroll]
