...
# Safend
#
# SVN 1.7 support by Aaron Ballman (aaron@aaronballman.com)
#
# http://www.safend.com
#
...
sub GatherFileInformation {
    ...
    my $ParentDirectory = dirname($SourceRoot);
    ...
    while ($curline = <$hProcess>) {
        ...
        if ( $curline =~ m/^Path:[\s\t]*(.*)$/i ) {
            ...
            $LocalFile = "$ParentDirectory\\$local_path";
            ...
            while ($curline = <$hProcess>) {
                ...
                # If there is a Working Copy Root Path element, then we are working
                # with a newer version of svn where Path doesn't have the full path
                # any longer.
                #if ( $curline =~ m/Working Copy Root Path:[\s\t]*(.*)$/i ) {
                #    $LocalFile = lc ($1 . '\\' . $local_path);
                #}
