...
void ZEDGizmo::SetAxisLenght(float Size)
{
	AxisLenght = Size;
	DirtyGizmoFlag = true;
}

float ZEDGizmo::GetAxisLenght()
{
	return AxisLenght;
}

ZEVector3 ZEDGizmo::GetPositionChange()
{
	ZEVector3 Result;
...
void ZEDGizmo::BaseInitialize()
{
	GizmoMaterial = ZESimpleMaterial::CreateInstance();
}

void ZEDGizmo::BaseDeinitialize()
{
...
void ZEDGizmo::Draw(ZEDrawParameters* DrawParameters)
{
	...
	RenderCommand.Flags = (Mode == ZED_GM_ROTATE ? ZE_ROF_ENABLE_VIEW_PROJECTION_TRANSFORM : ZE_ROF_ENABLE_WORLD_TRANSFORM | ZE_ROF_ENABLE_VIEW_PROJECTION_TRANSFORM) | ZE_ROF_ENABLE_NO_Z_WRITE;
	...
	RenderCommand.Priority = INT_MAX;
	RenderCommand.Order = FLT_MAX;
...
void ZEDGizmo::StartMoveProjection(ZEDGizmoAxis Axis, const ZERay& InitialRay)
{
