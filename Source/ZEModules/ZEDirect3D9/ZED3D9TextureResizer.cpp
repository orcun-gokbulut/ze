...
bool ZED3D9TextureResizer::Process()
{
	...
	if(SrcInfo.Width != DestInfo.Width)
	{
		...
		Input->Create(SrcInfo.Width, SrcInfo.Height, 1, ZE_TPF_I8_4, false);
		...
		Output->Create(DestInfo.Width, SrcInfo.Height, 1, ZE_TPF_I8_4, true);
	...
	if(SrcInfo.Height != DestInfo.Height)
	{
		...
		// Create output texture
		Output = (ZED3D9Texture2D*)ZETexture2D::CreateInstance();
		Output->Create(DestInfo.Width, DestInfo.Height, 1, ZE_TPF_I8_4, true);
