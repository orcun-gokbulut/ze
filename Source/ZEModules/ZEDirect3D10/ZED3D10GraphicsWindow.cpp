...
/*******************************************************************************
 Zinek Engine - ZED3D10GraphicsWindow.cpp
 ------------------------------------------------------------------------------
...
void ZED3D10GraphicsWindow::ReleaseSwapChain()
{
	ZED3D_RELEASE(SwapChain);
}

bool ZED3D10GraphicsWindow::CreateSwapChain()
{
	ZED3D10GraphicsModule* Module = (ZED3D10GraphicsModule*)zeGraphics;
...
void ZED3D10GraphicsWindow::ReleaseBackBuffer()
{
	ZE_DESTROY(BackBuffer);
	ZE_DESTROY(DepthBuffer);
}

bool ZED3D10GraphicsWindow::CreateBackBuffer()
{
	HRESULT Result = S_FALSE;
	ID3D11Device* Device = ((ZED3D10GraphicsModule*)zeGraphics)->GetD3D10Device();
	...
	if (FAILED(Result))
	{
		zeCriticalError("Cannot create back buffer. Error: %d", Result);
		return false;
	}

	this->BackBuffer = new ZED3D10RenderTarget(Width, Height, 1, ZE_TPF_I8_4, ZE_TT_2D, RenderTarget);

	this->DepthBuffer = (ZED3D10DepthStencilBuffer*)ZEDepthStencilBuffer::CreateInstance();
	this->DepthBuffer->Create(Width, Height, ZE_DSPF_DEPTH24_STENCIL8);
...
bool ZED3D10GraphicsWindow::ManageFlagCreated()
{
	if (!CreateSwapChain())
		return false;
...
bool ZED3D10GraphicsWindow::ManageFlagDestroyed()
{
	ReleaseBackBuffer();
	ReleaseSwapChain();
...
bool ZED3D10GraphicsWindow::ManageFlagResized()
{
	ReleaseBackBuffer();

	UINT SwapShainFlags = DXGI_SWAP_CHAIN_FLAG_ALLOW_MODE_SWITCH;
	HRESULT Result = SwapChain->ResizeBuffers(0, Width, Height, DXGI_FORMAT_UNKNOWN, SwapShainFlags);
	if (FAILED(Result))
	{
...
bool ZED3D10GraphicsWindow::ManageFlagWindowed()
{
...
bool ZED3D10GraphicsWindow::ManageFlagFullScreen()
{
	...
	ZED3D10GraphicsModule* Module = (ZED3D10GraphicsModule*)zeGraphics;
	IDXGIOutput1* DXGIOutput = ((const ZED3D10GraphicsMonitor*)FullScreenMonitor)->GetDXGIOutput();
...
IDXGISwapChain1* ZED3D10GraphicsWindow::GetDXGISwapChain() const
{
	return SwapChain;
}

bool ZED3D10GraphicsWindow::Update()
{
...
bool ZED3D10GraphicsWindow::Present()
{
	//! Handle these errors
...
const ZERenderTarget* ZED3D10GraphicsWindow::GetBackBuffer()
{
	return BackBuffer;
}

const ZEDepthStencilBuffer* ZED3D10GraphicsWindow::GetDepthBuffer()
{
	return DepthBuffer;
}

ZED3D10GraphicsWindow::ZED3D10GraphicsWindow()
{
	SwapChain = NULL;
	BackBuffer = NULL;
	DepthBuffer = NULL;
