...
/*******************************************************************************
 Zinek Engine - ZED3D11GraphicsDevice.cpp
 ------------------------------------------------------------------------------
...
void ZED3D11GraphicsDevice::ApplyInputStates()
{
	bool Commit = false;
	ZEStatePool* StatePool = zeGraphics->GetStatePool();

	static ZEGRVertexLayout StaticLayout;
		
	ZESize LayoutHash = 0;
	ZEGRVertexLayout*	LayoutPtr = NULL;
		
	if (CurrentState.VertexLayout == NULL)
	{
		// Auto create layout
		bool Result = ZEGRVertexLayout::GenerateLayout(StaticLayout, CurrentState.VertexBuffers, CurrentState.VertexShader);
		if (!Result)
		{
...
void ZED3D11GraphicsDevice::ApplyVertexShaderStates()
{
	bool Commit = false;
	ZEStatePool* StatePool = zeGraphics->GetStatePool();
	...
	{
		ID3D11VertexShader* D3D10Shader = (CurrentState.VertexShader == NULL) ? NULL : ((ZED3D11VertexShader*)CurrentState.VertexShader)->D3D10VertexShader;
	...
	for (ZESize I = 0; I < ZE_MAX_TEXTURE_SLOT; ++I)
	{
		...
		const ZEGRTexture* Texture = CurrentState.VertexShaderTextures[I];
...
void ZED3D11GraphicsDevice::ApplyGeometryShaderStates()
{
	bool Commit = false;
	ZEStatePool* StatePool = zeGraphics->GetStatePool();
	...
	{		
		ID3D11GeometryShader* D3D10Shader = (CurrentState.GeometryShader == NULL) ? NULL : ((ZED3D11GeometryShader*)CurrentState.GeometryShader)->D3D10GeometryShader;
	...
	{
		ID3D11ShaderResourceView* D3D10Resource = NULL;
		const ZEGRTexture* Texture = CurrentState.GeometryShaderTextures[I];
...
void ZED3D11GraphicsDevice::ApplyRasterizerStates()
{
	bool Commit = false;
	ZEStatePool* StatePool = zeGraphics->GetStatePool();
...
void ZED3D11GraphicsDevice::ApplyPixelShaderStates()
{
	bool Commit = false;
	ZEStatePool* StatePool = zeGraphics->GetStatePool();
	...
	{
		ID3D11PixelShader* D3D10Shader = (CurrentState.PixelShader == NULL) ? NULL : ((ZED3D11PixelShader*)CurrentState.PixelShader)->D3D10PixelShader;
	...
	{
		ID3D11ShaderResourceView* D3D10Resource = NULL;
		const ZEGRTexture* Texture = CurrentState.PixelShaderTextures[I];
...
void ZED3D11GraphicsDevice::ApplyOutputStates()
{
	bool Commit = false;
	ZEStatePool* StatePool = zeGraphics->GetStatePool();
...
void ZED3D11GraphicsDevice::ApplyStates()
{
	ApplyOutputStates();
...
ZED3D11GraphicsDevice::ZED3D11GraphicsDevice(ID3D11Device* D3D10Device, ZEUInt ContextIndex) : ContextIndex(ContextIndex)
{
	RenderTargetChanged = false;
...
ZED3D11GraphicsDevice::~ZED3D11GraphicsDevice()
{
