...
/*******************************************************************************
 Zinek Engine - ZED3D11Texture2D.cpp
 ------------------------------------------------------------------------------
...
bool ZED3D11Texture2D::UpdateWith(ZEUInt ShadowIndex)
{
...
const ID3D11ShaderResourceView* ZED3D11Texture2D::GetD3D10ResourceView() const
{
...
ZEGRRenderTarget* ZED3D11Texture2D::CreateRenderTarget(ZEUInt MipLevel) const
{
	...
	RenderTargetDesc.Format = GetD3D10PixelFormat(PixelFormat);
	...
	ZED3D11RenderTarget* RenderTarget = new ZED3D11RenderTarget(Width >> MipLevel, Height >> MipLevel, 1, PixelFormat, TextureType, D3D10RenderTargetView);
...
bool ZED3D11Texture2D::Create(ID3D11Texture2D* D3DTexture)
{
	zeDebugCheck(D3DTexture == NULL, "NULL pointer");
	...
	HRESULT Result = D3DDevices[0]->CreateShaderResourceView(D3DTexture, &ViewDesc, &D3D10ShaderResourceView);
	if(FAILED(Result))
	{
		zeError("D3D10 texture 2D shader resource view creation failed. ErrorCode: %d.", Result);
		return false;
	}

	D3D10Texture2D = D3DTexture;
...
bool ZED3D11Texture2D::CreateDynamic(ZEUInt Width, ZEUInt Height, ZEGRTextureFormat PixelFormat, ZETextureData* Data)
{
	...
	TextureDesc.Format = GetD3D10PixelFormat(PixelFormat);
