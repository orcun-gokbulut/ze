...
void ZELightProjective::Render(const ZERNRenderParameters* Parameters, const ZERNCommand* Command)
{
	...
	ZERNStageShadowing* StageShadowing = static_cast<ZERNStageShadowing*>(Parameters->Stage);
	this->ShadowMapIndex = StageShadowing->ShadowMapCount;
	const ZEGRDepthStencilBuffer* DepthBuffer = StageShadowing->ProjectiveShadowMaps->GetDepthStencilBuffer(false, StageShadowing->ShadowMapCount++);
