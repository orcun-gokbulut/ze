...
void ZEInteriorDebugDrawer::SetTarget(ZEEntity* Target)
{
	...
	if (ZEObjectDescription::CheckParent(ZEInterior::Description(), Target->GetDescription()) || (ZEInterior::Description() == Target->GetDescription()))
	{
...
bool ZEInteriorDebugDrawer::InitializeSelf()
{
	...
	if (InteriorMaterial == NULL)
		InteriorMaterial = ZEMaterialSimple::CreateInstance();

	InteriorRenderCommand.Material = InteriorMaterial;
//	InteriorRenderCommand.Flags = ZE_RCF_VIEW_PROJECTION_TRANSFORM;
	InteriorRenderCommand.VertexLayout = NULL;
	InteriorRenderCommand.PrimitiveType = ZE_PT_LINE_LIST;
