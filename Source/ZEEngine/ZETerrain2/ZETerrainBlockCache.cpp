...
void ZETerrainBlockCache::AddRequest(ZEUInt Level, ZEInt64 PositionX, ZEInt64 PositionY, ZETerrainBlockRequestType RequestType)
{
	...
	{
		if (RequestList[I].Level == Level && RequestList[I].PositionX == PositionX && RequestList[I].PositionY == PositionY)
		{
			if (RequestList[I].Type == ZE_TBRT_BUFFERED)
				RequestList[I].Type = RequestType;
			RequestListLock.Unlock();
			return;
		}
	}

	ZETerrainBlockRequest Request;
	Request.Level = Level;
...
bool ZETerrainBlockCache::Initialize()
{
	...
	if (Resource != NULL)
	{
		RequestList.SetCount(RequestListSize);
		BlockCache.SetCount(BlockCacheSize);
		IOThread.Run(NULL);
	}
