...
/*******************************************************************************
 Zinek Engine - ZEGraphicsWindow_Windows.cpp
 ------------------------------------------------------------------------------
...
static bool ExitValidation(ZEGraphicsWindow* Window)
{
...
bool ZEGraphicsCursor::GetPosition(ZEGraphicsWindow* Window, ZEInt& PosX, ZEInt& PosY)
{
...
bool ZEGraphicsCursor::CollisionCheck(const ZEGraphicsWindow* Window)
{
	POINT Pt;
...
{
	ZEGraphicsWindow* Window = NULL;

	if (uMsg == WM_CREATE)
	{
		LPCREATESTRUCT CreationParams = (LPCREATESTRUCT)lParam;
		Window = (ZEGraphicsWindow*)CreationParams->lpCreateParams;
...
{
	if (ZEGraphicsWindow::GetWindowCount() > 0)
		return true;
...
bool ZEGraphicsWindow::SetStyle(const ZEWindowStyle& Style)
{
	if (!IsInitialized())
	{
		this->Style = Style;
...
bool ZEGraphicsWindow::SetTitle(const char* String)
{
	if (!IsInitialized())
	{
		Title = String;
...
bool ZEGraphicsWindow::SetPosition(ZEInt X, ZEInt Y)
{
	if (!IsInitialized())
	{
		PositionX = X;
...
bool ZEGraphicsWindow::SetSize(ZEInt Width, ZEInt Height)
{
	if (!IsInitialized())
	{
		this->Width = Width;
...
bool ZEGraphicsWindow::SetVSynchEnabed(bool Value)
{
	VSynchEnabed = Value;
	return IsInitialized();
}
...
void ZEGraphicsWindow::Enable()
{
	::EnableWindow((HWND)Handle, true);
}

void ZEGraphicsWindow::Disable()
{
	::EnableWindow((HWND)Handle, false);
}

void ZEGraphicsWindow::Focus()
{
	if (!IsInitialized())
	{
		Focused = true;
...
bool ZEGraphicsWindow::Maximize()
{
	if (!IsInitialized())
	{
		Maximized = true;
...
bool ZEGraphicsWindow::Minimize()
{
	if (!IsInitialized())
	{
		Maximized = false;
...
static void ManageCursorLock(ZEGraphicsWindow* Window, bool Focused)
{	
...
bool ZEGraphicsWindow::SetCursorLock(bool Value)
{
...
ZESSize ZEGraphicsWindow::HandleMessage(ZEUInt32 Message, ZESize wParam, ZESSize lParam)
{
	ZESSize Return = 0;
...
bool ZEGraphicsWindow::Initialize()
{
	if (IsInitialized())
		return false;
...
bool ZEGraphicsWindow::Initialize(void* Handle)
{
	char TempString[128];
...
bool ZEGraphicsWindow::DeInitialize()
{
	if (Handle == NULL)
		return true;
