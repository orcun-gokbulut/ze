...
static void DownSample(float* Destination, float* Source, ZESize Size)
{
	...
	{
		float* SourceRowOdd = Source + Size * y * 2;
		float* SourceRowEven = Source + Size * (y + 1) * 2;
...
bool ZETerrainBlock::DebugDump(const ZEString& Directory)
{
	for (ZESize I = 0; I < MipmapCount; I++)
	{
		...
		if (Bitmap == NULL)
			return false;
		if (!FreeImage_Save(FIF_PNG, Bitmap, ZEFormat::Format("{4}/L{0}-X{1}-Y{2}-ML{3}", GetLevel(), GetPositionX(), GetPositionY(), I, Directory).ToCString()))
			return false;
