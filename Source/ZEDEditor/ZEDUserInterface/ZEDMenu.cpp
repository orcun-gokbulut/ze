...
void ZEDMenu::Update()
{
	...
	ZEString CurrentSection = ChildMenus[0]->GetSection();
	for (ZESize I = 0; I < ChildMenus.GetCount(); I++)
	{
		if (CurrentSection != ChildMenus[I]->GetSection())
		{
			if (MenuBar != NULL)
			...
			CurrentSection = ChildMenus[I]->GetSection();
		}
...
void ZEDMenu::SetSection(const ZEString& Section)
{
...
void ZEDMenu::AddMenu(const ZEString& Path, ZEDMenu* Menu)
{
	...
	if (!Current.IsEmpty())
	{
		for(ZESize I = 0; I < ChildMenus.GetCount(); I++)
		{
			if (ChildMenus[I]->GetSection() == Section && 
