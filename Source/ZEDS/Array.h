...
class ZEArray
{
	...
	public:
		...
		inline Type Dequeue()
		{
			ZEDS_ASSERT(Count == 0, "ZEArray::Dequeue operaion failed. There is no item in the queue.");
			Type Temp = Items[0];
			DeleteAt(0);
			return Temp;			
		}
		...
		inline Type Pop()
		{
			ZEDS_ASSERT(Count == 0, "ZEArray::Pop operaion failed. There is no item in the stack.");
			Type Temp = Items[0];
			DeleteAt(0);
			return Temp;
		}
		...
		{
			ZEDS_ASSERT(Index < 0 || Index >= Count, "ZEArray::SetItem operaion failed. Index is out of range. (0 <= Index < Count)");
		...
		{
			ZEDS_ASSERT(Index < 0 || Index > Count, "ZEArray::Insert operation failed. Index is out of range. (0 <= Index <= Count)");
			size_t N = 0;
		...
		{
			ZEDS_ASSERT(Index < 0 || Index >= Count, "ZEArray::DeleteAt operation failed. Index is out of range. (0 <= Index < Count)");
		...
		{
			ZEDS_ASSERT(Count < 0, "ZEArray::SetCount operation failed. Wrong Count value. (Count >= 0)");
		...
		{
			ZEDS_ASSERT(Count < 0, "ZEArray::Resize operation failed. Wrong Count value. (Count >= 0)");
